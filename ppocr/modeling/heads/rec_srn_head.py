...
class SRNPredict(object):
    ...
    def pvam(self, inputs, others):
        ...
        #pvam_features.stop_gradient = True
    ...
    def gsrm(self, pvam_features, others):
        ...
        #fluid.layers.Print(input = pvam_features)
        word_out = fluid.layers.fc(input=fluid.layers.reshape(pvam_features, [-1, c]),
                                  size=self.char_num,
                                  act="softmax")

        #fluid.layers.Print(input = word_out)
        return word_out
    ...
    def __call__(self, inputs, others, mode=None):
        ...
        word_out = self.gsrm(pvam_features, others)
        ...
        _, decoded_out = fluid.layers.topk(input=word_out, k=1)
        predicts = {'predict': word_out, 'decoded_out': decoded_out, 
                    'word_out': word_out, 'gsrm_out': word_out}  
