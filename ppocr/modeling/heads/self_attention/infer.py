...
def prepare_batch_input(insts, data_input_names, src_pad_idx, bos_idx, n_head,
    ...
    data_input_dict = dict(
        zip(data_input_names, [
            input, src_pos, src_slf_attn_bias, trg_word, init_score,
    ...
    for data_buffer in data_generator:
        ...
        try:
                ...
                for i in range(len(seq_ids.lod()[0]) -
                    ...
                    for j in range(end - start):  # for each candidate
                        ...
                        if _score > ref:
                            ret = _res
                            ref = _score
                        logging.debug("raw idx is" + str(np.squeeze(np.array(seq_ids)[sub_start:sub_end])))
            ...
            if args.use_py_reader:
                pyreader.reset()
            break
    logging.info('epoch %d ,infer accuracy is %f speed is %f s per pic'%(pass_id,1.0*cnt/(count+1e-9),(time.time()-start_time)/(count+1e-9)))
...
if __name__ == "__main__":
    ...
    vali_dg = dg.datagen(filelist, args.batch_size, vocab_fpath, train_imgshape,args.imgs_dir)
