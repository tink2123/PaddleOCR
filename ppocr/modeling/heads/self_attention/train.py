...
def pad_batch_data(insts,
                   ...
                   return_max_len=True,
                   return_num_token=False,max_len=0):
    """
    ...
    if return_attn_bias:
        if is_target:
            ...
            slf_attn_bias_data2 = np.tril(slf_attn_bias_data,
                                         -1).reshape([-1, 1, max_len, max_len])
            slf_attn_bias_data2 = np.tile(slf_attn_bias_data2,
                                         [1, n_head, 1, 1]) * [-1e9]
             
            return_list += [slf_attn_bias_data1.astype("float32") , slf_attn_bias_data2.astype("float32")]
        else:
            ...
            slf_attn_bias_data = np.tile(
                ...
                [1, 1, max_len, 1])
            slf_attn_bias_data[:,0,range(0,max_len),range(0,max_len)] = -1e9
...
def prepare_batch_input(insts, data_input_names, src_pad_idx, trg_pad_idx,
    ...
    _, src_pos, src_slf_attn_bias, src_max_len = pad_batch_data(
        [[0]*src_conv_seq_len for i in range(len(input))], src_pad_idx, n_head, is_target=False)
    # src_word = input
    ...
    for data_buffer in data_generator:
        try:
            ...
            feed_dict_list = prepare_feed_dict_list(data_buffer, False,
                                                    dev_count)
            if args.use_parallel_exe:
                outs = test_exe.run(fetch_list=[sum_cost.name, token_num.name,predict],
                                feed=feed_dict_list)
            else:
                outs = exe.run(test_prog,fetch_list=[sum_cost.name, token_num.name, predict],
                                    feed=feed_dict_list)
...
def train_loop(exe,
               ...
               nccl2_trainer_id=0,args= None,
               train_dg=None
               ,pass_id=0,vali_dg=None):
    ...
    for pass_id in range(1, TrainTaskConfig.pass_num):
        for data_buffer in data_generator:
            try:
                feed_dict_list = prepare_feed_dict_list(data_buffer,
                                                        init_flag, dev_count)
                logging.debug("lbl_word is :"+str(feed_dict_list["lbl_word"]))
                if args.use_parallel_exe:
                    outs = train_exe.run(fetch_list=[sum_cost.name,token_num.name,predict.name],
                        feed=feed_dict_list)
                else:
                    outs = exe.run(train_prog,
                        # if step_idx % args.fetch_steps == 0 else [],
                                         fetch_list=[sum_cost.name,token_num.name,predict.name],
                        feed=feed_dict_list)
        ...
        if pass_id%args.save_steps ==0:
            ...
            fluid.io.save_persistables(
                exe,
                os.path.join(TrainTaskConfig.ckpt_dir,"pass_" + str(pass_id) + ".ckpt.model"),
                train_prog)
