...
class datagen():
    def __init__(self,filepath,batchsize,vocab_fpath,train_imgshape,imgs_dir):
        self.imgs_dir = imgs_dir
        self.char2idx, self.idx2char = load_vocab(vocab_fpath)
        self.filepath = filepath
        self.filelist =self.metafilerecord() #path,label,shape such as(\data\img.png,"helloworld",(115,30))
        ...
        self._reader = dg.train_reader(batchsize=batchsize,
    ...
    def __getitem__(self, item):
        input = np.zeros((self.batchsize, self.train_imgshape[0],
                           self.train_imgshape[2],self.train_imgshape[1]),dtype='float32')
        y = []
        decoder_input =[]
        labels = []
        ...
        for j in range(self.batchsize):
            ...
            img = self.read(file)
            input[j] = img.transpose([2,0,1]).astype(np.float32)#w,h,c ==> c,w,h
...
if __name__=="__main__":
    ...
    train_dg = datagen(args.train_filelist, batchsize, vocab_fpath, train_imgshape,args.imgs_dir)
