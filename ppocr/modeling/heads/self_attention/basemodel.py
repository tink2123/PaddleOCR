...
class CRNN():
    ...
    def ocr_convs(self,img,
        ...
        #32
        x = fluid.layers.conv2d(img,64,(3,3),padding=1,act='relu',use_cudnn=use_cudnn,param_attr=w1, bias_attr=b,name="backbone_block1_conv1")
        x = fluid.layers.pool2d(x,(2,2),"max",pool_stride=2,use_cudnn=use_cudnn,name="backbone_block1_pool1")
        #16
        x = fluid.layers.conv2d(x, 128, (3, 3), padding=1, act='relu', use_cudnn=use_cudnn,
                                param_attr=w1, bias_attr=b,name="backbone_block2_conv1")
        x = fluid.layers.pool2d(x, (2, 2), "max",pool_stride=2, use_cudnn=use_cudnn, name="backbone_block2_pool1")
        #8
        x = fluid.layers.conv2d(x, 256, (3, 3), padding=1, act='relu', use_cudnn=use_cudnn,param_attr=w1, bias_attr=b,name="backbone_block3_conv1")
        x = fluid.layers.batch_norm(x,param_attr=w1,bias_attr=b,name="backbone_block3_bn1")
        x = fluid.layers.conv2d(x, 256, (3, 3), padding=1, act='relu', use_cudnn=use_cudnn,param_attr=w1, bias_attr=b,name="backbone_block3_conv2")
        x = fluid.layers.batch_norm(x, param_attr=w1, bias_attr=b,name="backbone_block3_bn2")
        x = fluid.layers.pool2d(x, (2, 2), "max",pool_stride=(2,1),use_cudnn=use_cudnn, name="backbone_block3_pool1")
        #4
        x = fluid.layers.conv2d(x, 512, (3, 3), padding=1, act='relu', use_cudnn=use_cudnn,
                                param_attr=w1, bias_attr=b,name="backbone_block4_conv1")
        x = fluid.layers.batch_norm(x,  param_attr=w1, bias_attr=b, name="backbone_block3_bn1")
        x = fluid.layers.conv2d(x, 512, (3, 3), padding=1, act='relu', use_cudnn=use_cudnn,
                                param_attr=w1, bias_attr=b,name="backbone_block4_conv2")
        x = fluid.layers.batch_norm(x, param_attr=w1, bias_attr=b, name="backbone_block3_bn2")
        x = fluid.layers.pool2d(x, (2, 2), "max", pool_stride=(2, 1), use_cudnn=use_cudnn,
                                name="backbone_block3_pool1")
        #2
        x = fluid.layers.conv2d(x, 512, (2, 2), padding=0, act='relu', use_cudnn=use_cudnn,
                                param_attr=w1, bias_attr=b,name="backbone_block5_conv1")
        x = fluid.layers.batch_norm(x, param_attr=w1, bias_attr=b, name="backbone_block5_bn1")
        #x shape is [512,1,*]
...
class conv8():
    ...
    def net(self, input, class_dim=1000, regularizer=None, gradient_clip=None, is_test=False):
        b = fluid.ParamAttr(
            regularizer=regularizer,
            gradient_clip=gradient_clip,
            initializer=fluid.initializer.Normal(0.0, 0.0))
        ...
        conv2 = conv_bn_pool(
            ...
            tmp, 2, [16, 16], is_test=is_test)
        # conv2 = self.ATT(conv2, atteMap)
        # print(tmp)
        # ====group 2======

        conv3 = conv_bn_pool(
            conv2, 2, [32, 32], is_test=is_test)
        # print(tmp)
...
if __name__=="__main__":
    ...
    with fluid.program_guard(train_prog, startup_prog):
        img = fluid.layers.data(name='image', shape=[1, 48, 512], dtype='float32')
        res=myconv8.net(img)
        print(res)
